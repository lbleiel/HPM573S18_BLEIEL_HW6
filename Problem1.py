...
class MultiGame:
    ...
    def simulate(self):
        for i in range(len(self._ids)):
            # create game
            game1 = SetOfGames(self._ids[i], self._probHeads[i], self._nGames[i])
            # simulate game
            output = game1.simulate()
            # store average rewards for this game
            self._expectedRewards.append(game1.get_ave_reward())
            # store average rewards for this game
            self._meanExpectedRewards.append(output.get_ave_Reward())
    ...
    def get_all_mean_rewards(self):
        return self._expectedRewards

    def get_overall_mean_rewards(self):
        return self._sumStat_meanExpectedReward.get_mean()
...
###########################################################################
Casino = SetOfGames(ids=1, prob_head=0.5, n_games=1000)
...
print("Gambler:")
Gambler = SetOfGames(ids=1, prob_head=0.5, n_games=10)
print("For a gambler playing the game 10 times, the average expected "
      "reward is:", Gambler.get_ave_reward())
print("For the gambler, the 95% PI of "
      "average expected reward is:",GameOutcomes(simulated_game=Gambler).get_PI_reward(ALPHA))
